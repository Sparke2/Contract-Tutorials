...
Если все сделано правильно, то мы сможем читать функции из контракта или послать транзакцию в контракт через этот интерфейс. Те функции, которые были помечены как `constant returns` в коде контракта доступны для чтения.

![alt text](https://github.com/Sparke2/Contract-Tutorials/blob/master/Wallet_10.jpg)

Чтобы вызвать какую-либо из функций контракта, которая изменяет его состояние (к примеру задает значение переменной `X`), необходимо разблокировать кошелек и так же послать в контракт транзакцию. Сделать это можно через интерфейс MyEtherWallet, нажав кнопку Write и указав необходимое количество газа.

**Газ расходуется на исполняемые вычисления контракта!** Если функция выполняет простые действия (в нашем случае присваивает значение переменной), они будут расходовать мало газа. Функции, выполняющие более сложные вычисления (к примеру производящие цикл вычислений), будут расходовать больше газа и, соответственно, стоить дороже. Если вы приложите к транзакции меньше газа, чем требуется для полного исполнения вызванной функции, то весь газ будет израсходован, но никаких изменений в контракт не будет внесено.

Ориентировочная стоимость вычислений в сети Эфириум - 20000 газа за 1 миллисекунду времени, затрачиваемую на исполнение.

![alt text](https://github.com/Sparke2/Contract-Tutorials/blob/master/Wallet_11.jpg)
